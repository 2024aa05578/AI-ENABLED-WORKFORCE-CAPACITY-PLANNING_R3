...
def data_input_section():
    ...
    with tab3:
        ...
        assumptions["target_utilization_pct"] = c2.number_input(
            ...
            min_value=10.0,
            max_value=100.0,
            value=float(assumptions.get("target_utilization_pct", 85.0)),
            step=1.0
        )
        assumptions["shrinkage_pct"] = c3.number_input(
            ...
            min_value=0.0,
            max_value=80.0,
            value=float(assumptions.get("shrinkage_pct", 12.0)),
            step=0.5
        )

        c4, c5 = st.columns(2)
        assumptions["skill_buffer_pct"] = c4.number_input(
            "Skill / Training Buffer %",
            min_value=0.0,
            max_value=80.0,
            value=float(assumptions.get("skill_buffer_pct", 10.0)),
            step=0.5
        )
        assumptions["default_monthly_capacity_per_engineer"] = c5.number_input(
            "Default Monthly WO Capacity per Engineer",
            min_value=1,
            max_value=1000,
